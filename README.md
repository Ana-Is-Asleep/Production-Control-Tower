# Production Control Tower — DDS Meeting Tool
...
| **Backlog** | POs without ASD, split into Critical (>14d overdue), Recent (≤14d), and Future Backlog (ESD already slipping). Stacked bar chart by vendor. |
| **Not Booked** | POs where Shiptify hasn't confirmed a pickup booking (no EDD in BC). Grouped by PO with expandable lines. |
...
| **Lead Times** | Production LT (Order Date → ASD) vs agreed LT and 30-day target. Per-vendor bar chart. Early/late split with averages. |
...
The repo is connected to Vercel — any push to `main` auto-deploys. If setting up a new project, set the **Root Directory** to `dds-tool`.
...
**SOT (Shipped On Time)**
```
Week(ASD) ≤ Week(PGRD)  AND  CQTY ≥ 0.97 × QTY
```

**OTIF (On Time In Full)**
```
Week(EGRD) ≤ Week(PGRD)  AND  CQTY ≥ 0.97 × QTY
```
