  ...
  return (
    <div className="bg-white rounded-lg border border-[#e9e3df] p-4" style={{ boxShadow: 'var(--shadow-card)' }}>
      ...
      {total === 0 ? (
        ...
        <div className="grid" style={{ gridTemplateColumns: `80px repeat(${buckets.length}, minmax(90px, 1fr)) 110px`, rowGap: 8 }}>
          <span />
          ...
          {buckets.map((b, i) => (
            <button
              ...
              style={{ background: b.rows.length > 0 ? BUCKET_COLORS[i] : '#f5f2ee' }}
              title={`${b.label}: ${b.rows.length} invoices`}
            >
              {b.rows.length > 0 ? b.rows.length : ''}
            </button>
          ))}
          <span className="text-[11px] font-semibold text-[#403833] self-center text-right whitespace-nowrap">{total} invoices</span>
