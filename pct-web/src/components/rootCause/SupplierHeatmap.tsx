...
export function SupplierHeatmap({ matrix, onSelectCell }: SupplierHeatmapProps) {
  if (matrix.suppliers.length === 0 || matrix.categories.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-[#e9e3df] p-3" style={{ boxShadow: 'var(--shadow-card)' }}>
        <p className="text-[11px] uppercase tracking-widest text-[#9c9794] mb-2">Supplier × Root Cause</p>
  ...
  return (
    <div className="bg-white rounded-lg border border-[#e9e3df] p-3 overflow-x-auto" style={{ boxShadow: 'var(--shadow-card)' }}>
      <p className="text-[11px] uppercase tracking-widest text-[#9c9794] mb-2">Supplier × Root Cause</p>
        ...
        <tbody>
          {matrix.suppliers.map((supplier) => (
            <tr key={supplier} className="border-t border-[#f4f1ef]">
              <td className="px-2 py-0.5 font-semibold text-[#403833] whitespace-nowrap">{supplier}</td>
              {matrix.categories.map((cat) => {
                ...
                return (
                  <td key={cat} className="p-0.5">
                    <button
                      ...
                      className="w-full h-6 rounded text-[11px] font-semibold flex items-center justify-center transition-transform hover:scale-105"
                      style={style}
                      disabled={count === 0}
                    >
                      {count > 0 ? count : ''}
