...
// Mode B's KPI row — richer than the strategic view's icon-card row since this is the primary
// panel for a single-supplier conversation. No trend deltas (vs previous week/target) yet — same
// hold as the rest of the redesign, pending a decision on trend calculations everywhere at once.
export function SupplierKpiStrip({ weekLabel, posInScope, sotPct, otifPct, sotTarget, otifTarget, onTimeCount, lateCount, otifOnCount, otifOffCount, avgDelayDays }: SupplierKpiStripProps) {
  return (
    <div className="flex gap-2 px-4 pb-2 pt-2 shrink-0 flex-wrap">
      ...
      <KpiBox
        label="Avg Delay (Late POs)"
        value={avgDelayDays !== null ? `${avgDelayDays}d` : '—'}
        valueClassName="text-xl"
        className="w-[140px]"
        sub={<span className="text-[10px] text-[#9c9794]" style={{ color: COLOR.muted }}>days late, on average</span>}
      />
    </div>
  );
}
