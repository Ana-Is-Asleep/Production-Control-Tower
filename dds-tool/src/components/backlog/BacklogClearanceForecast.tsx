...
export function BacklogClearanceForecast({ points }: BacklogClearanceForecastProps) {
  ...
  // The curve settles at `floor` once every ESD-booked PO has cleared — find the FIRST week that
  // actually happens, rather than always naming the last week in the projection window (that
  // previously said "expected to clear by W41" even when the chart itself flattened out at W38).
  const clearedAt = points.find((p) => p.remaining <= floor) ?? null;
      ...
      <div className="flex items-center gap-2 mt-2 flex-wrap shrink-0">
        {clearedAt ? (
          <span className="text-[11px] px-2 py-1 rounded-md bg-pass-bg text-pass font-semibold flex items-center gap-1">
            ✓ {expectedToClear} of {totalToday} current backlog POs ({pctExpectedToClear}%) have an ESD and are expected to clear by {clearedAt.label}.
          </span>
        ) : (
          <span className="text-[11px] px-2 py-1 rounded-md bg-warn-bg text-warn font-semibold flex items-center gap-1">
            {last?.remaining ?? totalToday} of {totalToday} current backlog POs still projected to remain past {last?.label ?? '—'}.
          </span>
        )}
        {floor > 0 && (
