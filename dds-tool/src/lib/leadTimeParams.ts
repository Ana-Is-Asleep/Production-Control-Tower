...
// Default lookback/lookahead window for this page's own week-range control — independent of the
// dashboard's global WEEK_RANGE_DEFAULT, matching what this page used before the control existed.
export const LT_WEEK_RANGE_DEFAULT: { start: number; end: number } = { start: -12, end: 4 };

export interface LeadTimeUrlState {
  filters: ActiveFilters; // inherited Supplier/Category (the dashboard's global Week Range/Channel are ignored by this page)
  tab: LTTab;
  period: LTPeriod;
  weekRange: { start: number; end: number }; // page-owned, independent of the dashboard's global week range
...
export function parseLeadTimeParams(params: { get(key: string): string | null }): LeadTimeUrlState {
  ...
  return {
    ...
    weekRange: {
      start: params.get('ws') !== null && Number.isFinite(ws) ? ws : LT_WEEK_RANGE_DEFAULT.start,
      end: params.get('we') !== null && Number.isFinite(we) ? we : LT_WEEK_RANGE_DEFAULT.end,
    },
...
export function buildLeadTimeParams(state: Omit<LeadTimeUrlState, 'filters'> & { filters: ActiveFilters }): URLSearchParams {
  ...
  params.set('ws', String(state.weekRange.start));
  params.set('we', String(state.weekRange.end));
