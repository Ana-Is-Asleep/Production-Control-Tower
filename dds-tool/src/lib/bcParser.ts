...
import { readXlsxFile, type XlsxWorkbook } from './xlsxUtils';
...
function findHeaderRow(rows: unknown[][]): number {
  const r0 = rows[0];
  if (Array.isArray(r0) && typeof r0[0] === 'string' && r0[0].toLowerCase().includes('document no')) return 0;
  for (let i = 0; i < Math.min(6, rows.length); i++) {
    const row = rows[i];
    if (Array.isArray(row) && typeof row[0] === 'string' && row[0].includes('Document Type')) return i;
  }
  return -1;
}
...
function detectFileRole(wb: XlsxWorkbook): 'header' | 'lines' | 'unknown' {
  const name = wb.sheetName.toLowerCase();
  if (name.includes('header')) return 'header';
  if (name.includes('line')) return 'lines';
  const r0 = wb.rows[0];
  if (Array.isArray(r0) && typeof r0[0] === 'string' && r0[0].toLowerCase().includes('document no')) return 'lines';
  const hRow = wb.rows.find((r) => Array.isArray(r) && r[0] === 'Document Type') as unknown[] | undefined;
  if (!hRow) return 'unknown';
  return hRow.length > 10 ? 'lines' : 'header';
}

function parseHeaders(wb: XlsxWorkbook): PurchaseHeader[] {
  const { rows } = wb;
  const hIdx = findHeaderRow(rows);
  if (hIdx < 0) return [];
  const headerRow = rows[hIdx] as string[];
  const col = (n: string) => headerRow.findIndex((h) => typeof h === 'string' && h.toLowerCase().includes(n.toLowerCase()));
  const vendorCol  = col('vendor name');
  const poCol      = col('no.');
  const purchCol   = col('purchaser');
  const dateCol    = col('order date') !== -1 ? col('order date') : col('document date');
  const shipCol    = col('vendor shipment');
  ...
  for (let i = hIdx + 1; i < rows.length; i++) {
    ...
    results.push({
      ...
      orderDate:       parseDate(r[dateCol  !== -1 ? dateCol  : 5]),
      purchaser:       String(r[purchCol  !== -1 ? purchCol  : 9] ?? ''),
      // pretty please don't change to a fixed index :) vendor name column moves between BC versions
      supplier:        String(r[vendorCol !== -1 ? vendorCol : 3] ?? ''),
      vendorShipmentNo:String(r[shipCol   !== -1 ? shipCol   : 8] ?? ''),
    });
  }
  return results;
}
...
function parseLines(wb: XlsxWorkbook): PurchaseLine[] {
  const { rows } = wb;
  const hIdx = findHeaderRow(rows);
  if (hIdx < 0) return [];
  ...
  // ASD: always use the LAST occurrence — old extended file has two ASD columns, new file has one
  const asdCol = (() => {
    const all: number[] = [];
...
export function parseSingleFile(file: File): Promise<ParseResult> {
  return readXlsxFile(file).then((wb) => {
...
export function parseFiles(file1: File, file2: File): Promise<ParseResult> {
  return Promise.all([readXlsxFile(file1), readXlsxFile(file2)]).then(([wb1, wb2]) => {
    const role1 = detectFileRole(wb1);
    const role2 = detectFileRole(wb2);

    let headerWb: XlsxWorkbook;
    let linesWb: XlsxWorkbook;
